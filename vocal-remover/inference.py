...
class Separator(object):
    ...
    def _separate(self, X_spec_pad, roi_size):
        ...
        with torch.no_grad():
            mask_list = []
            total_batches = len(range(0, patches, self.batchsize))
            for idx, i in enumerate(range(0, patches, self.batchsize)):
                ...
                # ✅ Stream actual progress
                progress = int(((idx + 1) / total_batches) * 100)
...
def main():
    ...
    output_dir = args.output_dir.rstrip('/') + '/' if args.output_dir else ''
    if output_dir:
