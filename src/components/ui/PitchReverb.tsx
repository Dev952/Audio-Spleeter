...
export default function PitchReverb({
  ...
  const [reverb, setReverb] = useState(0); // 0-10 scale for FFmpeg
  ...
  // Build waveform for original preview with seeking capability
  useEffect(() => {
  ...
  // Build waveform for processed audio with seeking
  useEffect(() => {
    if (!processedWaveRef.current || !processedAudioUrl) return;
    if (processedWSRef.current) {
      processedWSRef.current.destroy();
      processedWSRef.current = null;
    }
  ...
  // Update current BPM when speed or original BPM changes
  useEffect(() => {
    if (originalBpm) {
  ...
  // Debounced analysis when pitch changes
  ...
  // FIXED: Only show one green toast on file upload
  const handleFileUpload = (file: File) => {
    ...
    resetEffects(false); // Don't show reset toast during file upload

    // Only show this one green toast
  ...
  const handleApplyEffects = async () => {
    ...
    // FIXED: Custom styled processing toast with rounded corners
    toast("Processing...", {
    ...
    try {
      ...
      if (localFile) {
        ...
        formData.append("reverb", String(reverb)); // 0-10 scale
  ...
  // FIXED: Added parameter to control toast display
  const resetEffects = (showToast = true) => {
    ...
    // Only show toast when manually clicked, not during file upload
    if (showToast) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-purple-500/30 shadow-2xl">
        <CardContent className="p-8">
          <h2 className="text-3xl font-bold text-white mb-8 text-center bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
          ...
          {effectiveAudioUrl && (
            <>
              ...
              <div className="space-y-8 mb-8">
                ...
                {/* Reverb Control - FFmpeg 0-10 Scale */}
                <div className="p-6 bg-gray-800/30 rounded-xl border border-purple-500/20">
                  <div className="flex justify-between items-center mb-4">
                    <label className="text-purple-300 font-semibold text-lg">
                      🔊 Reverb
              ...
              <div className="flex flex-wrap gap-4 justify-center mb-8">
                ...
                <Button
                  ...
                  disabled={isProcessing}
                  className="cursor-pointer bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-8 py-3 text-lg font-semibold"
                >
                  {isProcessing ? (
                    <>
